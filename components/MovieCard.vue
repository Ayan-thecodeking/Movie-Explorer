...
.movie { display: flex; flex-direction: column; position: relative; }
...
.movie-img {
  ...
  .review {
    ...
    background: #c92502; color: #fff; border-radius: 0 0 12px 0; font-weight: 700;
  }
  .overview {
    line-height: 1.45; position: absolute; bottom: 0; left: 0; right: 0;
    background: rgba(0,0,0,0.78); padding: 12px; color: #fff;
    transform: translateY(100%); transition: transform .25s ease-in-out;
    max-height: 70%;
    overflow: auto;
  }
  &:hover .overview { transform: translateY(0); }

  .fav-btn {
    position: absolute; top: 10px; right: 10px;
    background: rgba(0,0,0,0.5); color: #fff; border: 1px solid #fff3; border-radius: 999px;
...
.info {
  margin-top: 10px;
  .title { color: #fff; font-size: 18px; margin: 0 0 4px; }
  .release { color: #c9c9c9; margin: 0 0 8px; }
}
</style>
